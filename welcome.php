...
$data = $_GET['data'];
$sql = "SELECT * FROM upload_products WHERE id = $data";
$result = mysqli_query($con, $sql);
if($result && mysqli_num_rows($result) > 0) {
    $row = mysqli_fetch_assoc($result);
    echo '<div class="container">
    <div class="jumbotron">
      <h1 class="display-4 text-center text-success">Welcome '.$row['p_name'].'</h1>
      <div style="display:flex;flex-direction:row;justify-content:space-between;align-item:center;">
      <p class=" red text-danger">Your  Order Id is:  '.$row['id'].'</p>
      ...
      <hr class="my-4">
      <p class="lead"></p>
      <a class="btn btn-dark btn-lg" href="check_product_status.php" role="button"> Back</a>
    </div>
    </div>';
}
?>
